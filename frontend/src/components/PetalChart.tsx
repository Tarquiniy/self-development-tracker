...
/**
 * PetalChart — overflow-safe, mobile-first.
 * Container: width:100%, maxWidth: min(size, 100%), aspect-ratio:1/1, overflow:hidden, margin:auto.
...
type Props = {
  ...
  size?: number; // preferred max width in px
...
export default function PetalChart(props: Props) {
  const {
    ...
    onSelectCategory,
    className,
    ariaLabel = "Лепестковая диаграмма"
  } = props;
  ...
  // fixed internal geometry based on `size` (no window usage)
  const W = Math.max(120, Number(size) || 360);
  const H = W;
  const pad = Math.max(20, Math.round(W * 0.08));
  const viewW = W + pad * 2;
  const viewH = H + pad * 2;
  const outerRadius = Math.max(22, Math.min(W, H) * 0.44);
  const innerRadius = Math.max(6, Math.min(W, H) * 0.05);

  const DEFAULT_COLORS = [
    "#FF6EC7", "#5A9CFF", "#7CE389", "#FFD59A",
    "#FF8A6B", "#9A7CFF", "#6AD3FF", "#FFB4E6",
  ];

  const petals: Petal[] = useMemo(() => {
    try {
      const normalized = catsInput.map((c: any, i: number) => {
      ...
      const globalMax = Math.max(1, ...normalized.map(x => (x.max && x.max > 0 ? x.max : Math.max(0, x.value))));
      ...
      for (let i = 0; i < N; i++) {
        ...
        const width = Math.max(12, Math.min(W, H) * 0.12);
        ...
        const basePath = `M 0 0 C ${cp1x.toFixed(3)} ${cp1y.toFixed(3)} ${width.toFixed(3)} ${(-outerRadius * 0.6).toFixed(3)} 0 ${tipY.toFixed(3)} C ${(-width).toFixed(3)} ${(-outerRadius * 0.6).toFixed(3)} ${cp2x.toFixed(3)} ${cp2y.toFixed(3)} 0 0 Z`;
      ...
      try { (window as any).__DEBUG_PETAL_CHART_ERROR__ = { err: String(err), catsInput }; } catch {}
      return [];
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [JSON.stringify(catsInput), W, H, outerRadius]);

  const progressArray = petals.map(p => clamp(Number(p.progress ?? 0), 0, 1));
  ...
  if (!petals.length) {
    return (
      <div style={{ padding: 12, textAlign: "center", color: "#6b7b88", width: "100%" }}>
  ...
  async function addDeltaTo(id: string | null, delta: number) {
    ...
    if (p) {
      if (delta > 0 && p.max != null && Number(p.value ?? 0) >= Number(p.max)) return;
      if (delta < 0 && Number(p.value ?? 0) <= 0) return;
    }
    if (typeof onAddDelta === "function") {
      try { await onAddDelta(id, delta); } catch (e) { console.warn("onAddDelta failed", e); }
    }
  }

  const progressToRadius = (progress: number) => innerRadius + clamp(progress, 0, 1) * (outerRadius - innerRadius);
      ...
      >
        <defs>
          ...
          {petals.map(p => {
            ...
            return <clipPath id={clipId} key={clipId}><path d={p.basePath} /></clipPath>;
          })}
        </defs>
...
function sanitizeId(s: string) { return String(s).replace(/[^a-zA-Z0-9_-]/g, "_"); }
